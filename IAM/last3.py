"""
DAG unifié optimisé pour l'extraction IAM avec parallélisation.

Ce DAG détecte automatiquement le mode d'extraction et exécute
les extractions IAM en PARALLÈLE pour tous les comptes.
...
Avantages:
- Extraction PARALLÈLE pour tous les comptes (10 comptes = 10 tasks parallèles)
- Pas de duplication de code
- Workflow linéaire et simple
- Scalable jusqu'à des centaines de comptes
"""
...
# ============================================================================
# DAG Unifié Optimisé avec Parallélisation
# ============================================================================

@product_action(
    action_id=Path(__file__).stem,
    payload=UnifiedExtractIamPayload,
    tags=["account", "iam", "parallel"],
    doc_md="""
    # DAG Extract IAM (Optimized with Parallelization)
    
    Unified DAG for extracting IAM data from AWS accounts with parallel execution.
    ...
    ## Key Features
    - **Automatic mode detection** based on payload
    - **Parallel extraction**: Each account is processed in a separate task
    - **Scalable**: Handles 1 to 100+ accounts efficiently
    - **Unified workflow**: No code duplication
    
    ## Workflow
    1. Detect extraction mode (BY_NAMES or BY_FILTERS)
    2. Retrieve accounts from database
    3. Create temporary directory
    4. **Extract IAM in parallel** for all accounts (one task per account)
    5. Compress all results
    6. Upload to COS
    """
)
def dag_extract_iam_create() -> None:
    """
    DAG unifié optimisé avec extraction parallèle pour tous les comptes.
    """
    ...
    # ========================================================================
    # STEP 4: Extraction IAM PARALLÈLE (une task par compte)
    # ========================================================================
    ...
    def extract_iam_for_account(
        ...
        try:
            ...
            # Option: continuer avec les autres comptes ou arrêter
            # Pour l'instant, on lève l'erreur pour arrêter
            raise
    
    # ========================================================================
    # STEP 5: Agrégation et compression des résultats
    # ========================================================================
    ...
    # ========================================================================
    # DÉFINITION DU WORKFLOW (avec parallélisation)
    # ========================================================================
    ...
    # .expand() crée une task par élément de la liste
