...
def dag_extract_iam_create() -> None:
    ...
    # ========================================================================
    # DÉFINITION DU GRAPHE COMPLET (Solution simplifiée sans XCom complexe)
    # ========================================================================
    ...
    # Branches - Récupération des comptes
    ...
    # Workflow commun - Création du répertoire
    directory = create_tmp_directory()
    join_get_accounts >> directory
    
    # SOLUTION ULTRA-SIMPLE: 
    # On crée UNE SEULE task d'extraction qui récupère les accounts en interne
    
    @step
    def extract_all_accounts_parallel(
        directory_path: str,
        mode_info: Dict[str, Any],
        payload: UnifiedExtractIamPayload = depends(payload_dependency),
        db_session: SASession = depends(sqlalchemy_session_dependency),
        ...
        **kwargs
        ...
        """
        Récupère les accounts depuis XCom et lance les extractions.
        """
        ti = kwargs.get('ti')
        
        # Récupérer les accounts depuis la branche exécutée
        accounts = None
        ...
        # Si pas trouvé, essayer BY_FILTERS
        if not accounts:
            try:
        ...
        if not accounts:
            raise ValueError("No accounts found in XCom")
        
        logger.info(f"Processing {len(accounts)} accounts in parallel...")
        
        # Extraction pour tous les comptes
        extraction_results = []
        ...
        for idx, account in enumerate(accounts, start=1):
            ...
            logger.info(f"[{idx}/{len(accounts)}] Extracting: {account_name}")
        ...
        logger.info(f"Extraction completed: {len(extraction_results)} accounts processed")
        return extraction_results
    
    # Extraction (séquentielle pour l'instant, mais on peut paralléliser avec ThreadPoolExecutor si besoin)
    extraction_results = extract_all_accounts_parallel(directory, mode_info)
