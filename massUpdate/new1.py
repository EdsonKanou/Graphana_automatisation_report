...
def dag_parallel_update() -> None:
    ...
    def vault_and_secrets_group(updated_accounts: List[dict]):
        ...
        def get_buhub_account_details(
            ...
            logger.info(f"BUHUB account details retrieved: {buhub_account_details.model_dump_json(indent=4)}")
        ...
        def get_schematics_api_key(
            ...
            # Use GLBHUB account (fallback or default)
        ...
        def get_vault_token(
            ...
            return {"vault_token": vault.vault.token}
    ...
    def workspace_lookup_group(updated_accounts: List[dict], vault_data: dict):
        ...
        # Parallel workspace lookup using expand
        workspace_lookups = get_workspace_for_single_account.expand(
            account_info=updated_accounts,
            schematics_api_key=[vault_data["schematics_api_key"]] * len(updated_accounts)
        )
    ...
    def parallel_deployment_group(
        ...
        def deploy_single_account(
            ...
            try:
                ...
                variables["vault_token"] = TerraformVar(vault_token["vault_token"], True)
        ...
        # Parallel deployment using expand
        deployments = deploy_single_account.expand(
            account_workspace=account_workspaces,
            vault_token=[vault_data["vault_token"]] * len(account_workspaces),
            schematics_api_key=[vault_data["schematics_api_key"]] * len(account_workspaces)
        )
    ...
    # Phase 4: Parallel workspace lookup (reçoit updated_accounts et vault_data)
    account_workspaces = workspace_lookup_group(updated_accounts, vault_data)
