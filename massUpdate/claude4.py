...
def get_buhub_account_api_key(
    ...
    """
    Get BUHUB account API key for Schematics operations
    
    This follows your existing pattern of getting the BUHUB API key
    which is used to authenticate Schematics operations.
    
    Returns:
        str: API key for Schematics backend
    """
    logger.info("🔑 Retrieving BUHUB account API key...")
    
    # Check if any account is WKLAPP type (needs BUHUB)
    has_wklapp = any(
        acc.get("account_type") == "WKLAPP" 
        for acc in updated_accounts
    )
    
    if has_wklapp:
        # Get BUHUB account details
        # You need to determine which BUHUB to use based on code_bu
        # For now, using the first account's code_bu
        first_account = updated_accounts[0]
        code_bu = first_account.get("code_bu")
        ...
        buhub_account = get_account_by_type_and_bu(
            "BUHUB", 
            code_bu, 
            db_session  # You'll need to add this dependency
        )
        
        if buhub_account:
            # Get BUHUB API key from Vault
            api_key = vault.get_secret(
                f"{buhub_account.number}/account-owner",
                mount_point="ibmsid"
            )["api_key"]
            
            logger.info(f"✅ Using BUHUB account API key for code_bu: {code_bu}")
            return api_key
    
    # Fallback: Use GLBHUB account
    logger.info("✅ Using GLBHUB account API key (fallback)")
    api_key = vault.get_secret(
        "GHUB_ACCOUNTNUMBERS/account-owner",
        mount_point="ibmsid"
    )["api_key"]
...
def get_workspaces_for_accounts(
    ...
    # Initialize Schematics backend with API key
    tf = schematics_backend_dependency(schematics_api_key)
...
def deploy_single_account(
    ...
    # Initialize Schematics backend for this account
    tf = schematics_backend_dependency(schematics_api_key)
    ...
    with ThreadPoolExecutor(max_workers=max_parallel) as executor:
        ...
        futures = {
            executor.submit(
                ...
                account_workspace,
                payload,
                schematics_api_key,  # Pass API key instead of tf object
                db_session,
                vault
...
def dag_parallel_update():
    ...
    # Phase 5: Get BUHUB API key for Schematics operations
    schematics_api_key = get_buhub_account_api_key(updated_accounts)
    
    # Phase 6: Get Schematics workspaces for accounts
    ...
    # Phase 7: Deploy changes in parallel
    ...
    accounts_in_vault >> schematics_api_key
