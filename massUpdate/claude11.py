"""
DAG: Account Parallel Update - Solution XCom
=============================================

✅ Utilise XCom pour passer les données entre TaskGroups
✅ Pas de closures (ne fonctionne pas avec ton framework)
✅ Approche standard Airflow

Version: 4.0 - XCom Solution
"""
...
@product_action(
    ...
    tags=["account", "admin", "parallel", "update", "xcom-solution"],
    doc_md="DAG avec TaskGroups utilisant XCom pour le passage de données."
)
def dag_parallel_update() -> Dict:
    ...
    def database_preparation_group():
        ...
        def update_accounts_database(
            ...
            # ✅ IMPORTANT: Le step retourne les données
            # Airflow les stocke automatiquement dans XCom
    ...
    # ==========================================
    # SHARED STEP: VAULT SECRETS (Hors TaskGroup)
    # ==========================================
    ...
    def vault_and_secrets_group():
        ...
        def check_accounts_in_vault(
            # ✅ SOLUTION: Recevoir updated_accounts depuis le step précédent
            updated_accounts: list[dict],  # Vient de update_accounts_database
            vault: Vault = depends(vault_dependency),
        ) -> bool:
            """Verify all accounts have API keys in Vault"""
            
            # ✅ updated_accounts est maintenant une liste Python normale
        ...
        def get_buhub_account_details(
            # ✅ Reçoit updated_accounts depuis le step précédent
        ...
        # ✅ Flow: Chaque step reçoit updated_accounts via XCom
    ...
    def workspace_lookup_group():
        ...
        def get_workspaces_for_accounts(
            # ✅ Reçoit les données des steps précédents
            updated_accounts: list[dict],
    ...
    # Phase 2: Database (retourne updated_accounts via XCom)
    updated_accounts = database_preparation_group()
    
    # Phase 3: Vault secrets (hors groupe pour réutilisation)
    vault_secrets = get_vault_secrets()
    
    # Phase 4: Vault & secrets group
    # ✅ Les steps à l'intérieur reçoivent updated_accounts automatiquement via XCom
    schematics_api_key = vault_and_secrets_group()
    
    # Phase 5: Workspace lookup
    # ✅ Le step reçoit updated_accounts ET vault_secrets via XCom
    account_workspaces = workspace_lookup_group()
    
    # Phase 6: Parallel deployment
    # ✅ Le step reçoit account_workspaces ET vault_secrets via XCom
