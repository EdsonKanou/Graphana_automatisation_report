...
def dag_parallel_update() -> None:
    ...
    @step
    def validate_and_plan_workspace(
        buhub_group: dict,
        ...
        Cette fonction est appelée EN PARALLÈLE pour chaque buhub via .expand()
        Le tf_backend_adapter_dependency extraira automatiquement le 
        buhub_account_number depuis buhub_group.
        ...
        Returns:
            dict: Workspace details avec plan Terraform généré
        """
        ...
        logger.info(
            ...
            f"(reference realm: {first_realm_name}, {len(realms)} realms)"
        )
        ...
        try:
            ...
            logger.info(f"✓ Terraform plan generated successfully, status: {workspace_status}")
            ...
            logger.info(
                ...
                f"plan_status={workspace_status}, "
                f"{len(realms)} realms"
            )
            ...
            logger.error(
                f"✗ Failed to process BUHUB {buhub_account_number} "
    ...
    # ✅ PARALLÉLISATION avec .expand()
    # Crée UNE TÂCHE par buhub_group
    # Le tf_backend_adapter_dependency extraira buhub_account_number de chaque groupe
    workspace_results = validate_and_plan_workspace.expand(
